...
class Go2VelocityNode : public rclcpp::Node {
 public:
  Go2VelocityNode() : Node("go2_velocity_node") {
    ...
    declare_parameter<double>("kp_policy_hip", 20.0);
    declare_parameter<double>("kp_policy_thigh", 20.0);
    declare_parameter<double>("kp_policy_calf", 40.0);
    declare_parameter<double>("kd_policy_hip", 1.0);
    declare_parameter<double>("kd_policy_thigh", 1.0);
    declare_parameter<double>("kd_policy_calf", 2.0);
    declare_parameter<double>("kp_standup", 60.0);
    declare_parameter<double>("kd_standup", 5.0);
    ...
    // Per-joint policy gains, in SDK order (indexed by sdk_idx directly).
    // Isaac order is [hip, thigh, calf] per leg; SDK order is the same triple
    // per leg, just with legs reordered — so the (hip/thigh/calf) pattern is
    // identical in both orderings.
    {
      float kp_hip = get_parameter("kp_policy_hip").as_double();
      float kp_thigh = get_parameter("kp_policy_thigh").as_double();
      float kp_calf = get_parameter("kp_policy_calf").as_double();
      float kd_hip = get_parameter("kd_policy_hip").as_double();
      float kd_thigh = get_parameter("kd_policy_thigh").as_double();
      float kd_calf = get_parameter("kd_policy_calf").as_double();
      for (int leg = 0; leg < 4; ++leg) {
        kp_policy_sdk_[leg * 3 + 0] = kp_hip;
        kp_policy_sdk_[leg * 3 + 1] = kp_thigh;
        kp_policy_sdk_[leg * 3 + 2] = kp_calf;
        kd_policy_sdk_[leg * 3 + 0] = kd_hip;
        kd_policy_sdk_[leg * 3 + 1] = kd_thigh;
        kd_policy_sdk_[leg * 3 + 2] = kd_calf;
      }
    }
    kp_standup_ = static_cast<float>(get_parameter("kp_standup").as_double());
    kd_standup_ = static_cast<float>(get_parameter("kd_standup").as_double());
    ...
    // QoS: best-effort, depth 1 (matches Unitree SDK DDS defaults).
 ...
 private:
  ...
  void LowStateCb(const unitree_go::msg::LowState& msg) {
    ...
    latest_quat_[0] = msg.imu_state.quaternion[0]; // w
    latest_quat_[1] = msg.imu_state.quaternion[1]; // x
    latest_quat_[2] = msg.imu_state.quaternion[2]; // y
    latest_quat_[3] = msg.imu_state.quaternion[3]; // z
  ...
  void PublishTimerCb() {
    ...
    if (state_snapshot == State::kStandup) {
      ...
      for (int i = 0; i < kNumLegMotor; ++i) {
        ...
        cmd.motor_cmd[i].tau = 0.0f;
        cmd.motor_cmd[i].kp = kp_standup_;
        cmd.motor_cmd[i].kd = kd_standup_;
      }
      if (alpha >= 1.0) {
        std::lock_guard<std::mutex> lock(state_mu_);
      ...
      for (int i = 0; i < kNumLegMotor; ++i) {
        ...
        cmd.motor_cmd[i].dq = 0.0f;
        cmd.motor_cmd[i].tau = 0.0f;
        cmd.motor_cmd[i].kp = kp_policy_sdk_[i];
        cmd.motor_cmd[i].kd = kd_policy_sdk_[i];
      }
    }
  ...
  void PolicyTimerCb() {
    ...
    // Process: target_q_isaac[i] = raw[i] * scale + default_isaac[i]
    // Then remap to SDK order.
  ...
  void BuildObservation(std::vector<float>& obs) {
    ...
    // 11..22: joint_pos_rel in Isaac order
    // 23..34: joint_vel_rel in Isaac order (default vel is 0)
    for (int i = 0; i < kNumLegMotor; ++i) {
    ...
    // 35..46: last_action (raw, Isaac order)
  ...
  float action_scale_;
  float kp_standup_, kd_standup_;
  std::array<float, kNumLegMotor> kp_policy_sdk_;
  std::array<float, kNumLegMotor> kd_policy_sdk_;
